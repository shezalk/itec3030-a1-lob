...
public class Exchange {
	...
	public boolean validateOrder(Order o) {
        ...
        if (security == null) {
			System.err.println("Order validation: ticker " + o.getTicker().getTicker() + " not supported.");
			return (false);
		}
		...
		if ((o instanceof Ask) && (pos < o.getQuantity())) {
			System.err.println("Order validation: seller with ID " + trader.getID()+ " not enough shares of " + o.getTicker().getTicker() + ": has " + pos + " and tries to sell " + o.getQuantity());
			return (false);
		}
	...
	public void submitOrder(IOrder o, long time) {
        ...
        public OrderOutcome processOrder(Order o, long time) {
            // Implement order processing logic
            OrderOutcome outcome = new OrderOutcome(o);
            
            // Simulate processing (this should contain actual logic for processing orders)
            outcome.setUnfulfilledOrder(o); // Assume the order is not completely fulfilled
		...
		if (o instanceof Bid) {// Order is a bid
			...
			oOutcome = book.processOrder(o, time);
		...
		for (ITrade t:oOutcome.getResultingTrades()) {
			...
			accounts.getTraderAccount(seller).applyFee(sellerFee);
			//Apply the above fee to the account balance of the seller
			accounts.getTraderAccount(seller).applyTradePayment(t.getTradeValue());
			//Apply the trade payment to the account balance of the seller (they earned money)
			accounts.getTraderAccount(seller).updatePosition(t.getSecurity().getTicker(), -t.getQuantity());
			//Deduct the sold stocks from the position of the seller
			accounts.getTraderAccount(buyer).applyFee(buyerFee);
